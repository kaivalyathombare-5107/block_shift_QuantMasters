...
async function checkUpstashRateLimit(
  ...
  if (!response.ok) {
    throw new Error(`Upstash HTTP status ${response.status}`);
  }
  ...
  const ttlSeconds = typeof results[2]?.result === 'number' && results[2].result > 0
    ? results[2].result
    : windowSeconds;
...
export async function checkRateLimit(
  ...
  try {
    ...
    try {
      upstashUrl = serverEnv.upstashRedisUrl;
      upstashToken = serverEnv.upstashRedisToken;
    } catch {
      // fallback
    }
    ...
    if (upstashUrl && upstashToken) {
      try {
        return await checkUpstashRateLimit(
          upstashUrl,
          upstashToken,
          identifier,
          limit,
        ...
        console.warn(
          '[Rate Limit] Upstash check failed, falling back to in-memory store:',
          upstashError
        );
    ...
    console.error('[Rate Limit] Unexpected rate limit failure:', err);
    return {
      allowed: true,
      remaining: limit,
